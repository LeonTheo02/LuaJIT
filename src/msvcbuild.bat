...
@if "%1" neq "production" goto :BUILD
@shift
@set LJLIBNAME=lua51p.lib
@set LJCOMPILE=%LJCOMPILE% /Z7 /DLUA_USE_APICHECK /DLUA_USE_ASSERT /DPRODUCTION
:BUILD
...
@set DASMFLAGS=-D WIN -D NOFFI -D P64
@set LJARCH=x64
@minilua
@if errorlevel 8 goto :X64
@set DASMFLAGS=-D WIN -D JIT -D NOFFI
@set LJARCH=x86
@set LJCOMPILE=%LJCOMPILE% /arch:SSE2
@goto :JIT
:X64
@if "%1" neq "jit" goto :NOJIT
@shift
@set DASMFLAGS=%DASMFLAGS% -D JIT
@goto :JIT
:NOJIT
@set LJCOMPILE=%LJCOMPILE% /DLUAJIT_DISABLE_JIT
:JIT
...
%LJCOMPILE% ljamalg.c
@if errorlevel 1 goto :BAD
%LJLIB% /OUT:%LJLIBNAME% ljamalg.obj lj_*.obj lib_*.obj
@if errorlevel 1 goto :BAD
:MTDLL
